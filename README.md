...
└─────────────────┘    └─────────────────┘    └─────────────────┘    └─────────────────┘
                                                                    ...
                                                                    └─────────────────┘
                                                                             ...
                                                                             ▼
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Redis Cache   │◀───│ Anomaly Service │◀───│  Qdrant VDB     │◀───│  Vector Store   │
│    (:6379)      │    │   (:8003)       │    │   (:6333)       │    │  (Embeddings)   │
...
│  Kafka: 9092 │ Qdrant: 6333 │ Redis: 6379 │ Embedding: 8000 │ Scoring: 8003 │ Grafana: 3000 │
└─────────────────────────────────────────────────────────────────────────────────────────┘
```
...
│   ├── spark_to_qdrant.py             # Spark to Qdrant pipeline
...
│   ├── qdrant_entries.py              # Qdrant data viewer
│   ├── qdrant_embeddings.py           # Embedding analysis
│   ├── manage_qdrant.py               # Qdrant management utilities
│   ├── view_qdrant.py                 # Qdrant visualization
│   ├── analyze_embeddings.py          # Embedding analytics
│   ├── clear_qdrant.py                # Database cleanup
...
- **Models**: Decision Tree, MLP, SGD, Qdrant Similarity
...
- **Vector Similarity**: Qdrant-based nearest neighbor detection
