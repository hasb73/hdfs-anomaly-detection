...
Uses KNN, Decision Tree, and Single-Layer Feedforward Network (MLP with single hidden layer)
"""
...
class QdrantSimilarityDetector:
    """Qdrant-based anomaly detector using similarity scoring"""
    
    def __init__(self, collection_name="training_embeddings", host="localhost", port=6333):
    ...
    def fit(self, X_train, y_train):
        """Store training embeddings in Qdrant collection"""
        print("Setting up Qdrant similarity detector...")
        ...
        try:
            ...
            # Store normal embeddings (label=0) in Qdrant
            normal_embeddings = X_train[y_train == 0]
            normal_indices = np.where(y_train == 0)[0]
            
            # Use smaller batch size and limit to prevent timeout
            max_embeddings = min(10000, len(normal_embeddings))  # Reduced to 2000 max
            batch_size = 100  # Process in smaller batches
            
            print(f"Storing {max_embeddings} normal embeddings in batches of {batch_size}...")
            
            for batch_start in range(0, max_embeddings, batch_size):
                batch_end = min(batch_start + batch_size, max_embeddings)
                batch_embeddings = normal_embeddings[batch_start:batch_end]
                batch_indices = normal_indices[batch_start:batch_end]
                
                points = []
                for i, embedding in enumerate(batch_embeddings):
                    points.append(models.PointStruct(
                        id=int(batch_indices[i]),
                ...
                try:
                    ...
                    print(f"  Batch {batch_start//batch_size + 1}/{(max_embeddings-1)//batch_size + 1} stored")
            ...
            print(f"Successfully stored {actual_points} embeddings in Qdrant")
    ...
    def _tune_threshold(self, X_val, y_val, thresholds=[0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9]):
        """Tune similarity threshold for best F1 score"""
        print("Tuning Qdrant similarity threshold...")
        best_f1 = 0
        best_threshold = 0.7
    ...
    def predict(self, X):
        ...
        if not self.is_fitted:
            raise Exception("QdrantSimilarityDetector not fitted. Call fit() first.")
            ...
            for embedding in batch_X:
                try:
                    ...
                    if search_result.points:
                        ...
                        avg_similarity = np.mean([hit.score for hit in search_result.points])
...
def generate_phd_results_table(outdir, results, model_scores, model_weights, best_params, 
                              ensemble_precision, ensemble_recall, ensemble_f1, total_samples, anomaly_ratio):
    """Generate comprehensive results table for PhD documentation"""
...
"""
    ...
    model_name_mapping = {
        ...
        'qdrant_similarity': 'Qdrant Similarity'
    }
    
    for model_name in ['dt', 'mlp', 'sgd', 'qdrant_similarity']:
        if model_name in model_scores:
            ...
            if model_name == 'qdrant_similarity':
...
"""
    ...
    results_file = os.path.join(outdir, f'phd_results_table_{timestamp}.md')
    ...
    csv_file = os.path.join(outdir, f'phd_results_data_{timestamp}.csv')
    df.to_csv(csv_file, index=False)
    
    print(f"   PhD results table saved to: {results_file}")
    print(f"   PhD results CSV saved to: {csv_file}")
    print(f"   Use these files in your PhD thesis/papers!")

def train_line_level_ensemble_v2(X=None, y=None, outdir=None, sample_size=20000):
    """Train ensemble model V2 on line-level HDFS data with DT, MLP, SGD and Qdrant similarity"""
    ...
    print("Training ensemble models V2 (Decision Tree, Multi-Layer Perceptron, Stochastic Gradient Descent, Qdrant)...")
    ...
    # Add Qdrant Similarity Detector
    print(f"\n{'='*50}")
    print("Setting up Qdrant Similarity Detector...")
    print(f"{'='*50}")
    
    try:
        qdrant_detector = QdrantSimilarityDetector()
        qdrant_detector.fit(X_train_s, y_train)
        
        # Now tune threshold using validation data and get the F1, precision, recall scores
        threshold_f1, threshold_precision, threshold_recall = qdrant_detector._tune_threshold(X_val_s, y_val)
        
        # Use the threshold tuning metrics as the final scores (for consistency)
        print(f"Qdrant final metrics - Precision: {threshold_precision:.4f}, Recall: {threshold_recall:.4f}, F1: {threshold_f1:.4f}")
        
        trained_models['qdrant_similarity'] = qdrant_detector
        model_scores['qdrant_similarity'] = {
            'precision': threshold_precision, 
            'recall': threshold_recall, 
            'f1': threshold_f1
        }
        best_params['qdrant_similarity'] = {'threshold': qdrant_detector.threshold}
        
    except Exception as e:
        print(f"  Failed to setup Qdrant detector: {e}")
    ...
    results = {
        ...
        'training_info': {
            ...
            'model_types': ['Decision Tree', 'MLP', 'SGD', 'Qdrant Similarity']
        }
    }
    
    # Save ensemble (excluding Qdrant client which can't be pickled)
    models_to_save = {k: v for k, v in trained_models.items() if k != 'qdrant_similarity'}
    ...
    with open(os.path.join(outdir, 'training_metadata_v2.json'), 'w') as f:
        ...
        if 'qdrant_similarity' in trained_models:
            metadata['qdrant_config'] = {
                'collection_name': trained_models['qdrant_similarity'].collection_name,
                'threshold': trained_models['qdrant_similarity'].threshold,
                'host': 'localhost',
                'port': 6333
            }
    ...
    # Generate detailed results table for PhD documentation
    print(f"\nGenerating detailed results table for PhD documentation...")
    generate_phd_results_table(outdir, results, model_scores, model_weights, best_params, 
    ...
    print(f"   Model types: Decision Tree, Multi-Layer Perceptron, Stochastic Gradient Descent, Qdrant Similarity")
...
if __name__ == "__main__":
    ...
    # Train ensemble V2 with line-level data (KNN, DT, SLFN)
    results = train_line_level_ensemble_v2(sample_size=200000)
