...
class HDFSLineLevelLoader:
    ...
    def _create_line_level_labels(self):
        ...
        # Define anomaly-indicative event patterns
        anomaly_patterns = [
            'exception', 'error', 'fail', 'timeout', 'denied', 'refused',
            'corrupt', 'invalid', 'unable', 'cannot', 'interrupted',
            'abort', 'terminate', 'disconnect', 'lost', 'dead'
        ]
    ...
    def get_line_level_data(self, sample_size: Optional[int] = None) -> Tuple[List[str], List[int]]:
        """
        Get individual log lines with line-level anomaly labels
        Returns: (log_messages, labels) where labels are 0=Normal, 1=Anomaly
        """
    ...
    def save_line_level_data(self, output_file='hdfs_line_level_data.jsonl', max_records=10600568):
        ...
        messages, labels = self.get_line_level_data(sample_size=max_records)
...
if __name__ == "__main__":
    ...
    messages, labels = loader.get_line_level_data(sample_size=10600568)
